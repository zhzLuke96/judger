...
func getFromPidWin(pid int) string {
	stdout, _ := exec.Command("tasklist").Output()
	outarr := strings.Split(string(stdout), "\n")
	for _, v := range outarr {
		if strings.Index(v, fmt.Sprintf("%d", pid)) != -1 {
			return strings.Replace(strings.Fields(v)[4], ",", "", 1)
		}
	}
	return ""
}
...
func stat(pid int, statType string) (*SysInfo, error) {
	...
	if statType == "ps" {
		...
		mem := getFromPidWin(pid)
		if len(mem) == 0 {
			return sysInfo, errors.New("Can't find process with this PID: " + strconv.Itoa(pid))
		}
		sysInfo.CPU = -1
		// cont get cpu usage on windows system
		memtemp, _ := strconv.Atoi(mem)
		sysInfo.Memory = float64(memtemp) * 1024
	}
	return sysInfo, nil
